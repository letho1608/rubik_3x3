...
class MagicCube:
    def __init__(self, size, spacing = 2, offset = [0,0,0]):
        ...
        oppossite_moves = [[a, b, -c, -d] for item in reversed(self.testMoveSet.copy()) if len(item) == 4 for a, b, c, d in [item]]
...
def main():
    ...
    gluPerspective(20, (display[0] / display[1]), 0.1, 300.0)
    ...
    # Create rubiks cubes
    rubiks_cube = MagicCube(size=3, offset=[-4,-0,0], spacing=2.1)
